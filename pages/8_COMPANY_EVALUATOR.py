...
def classify_fundamental_and_speculation(data, score):
    speculative_flags = []
    strong_fund = (data["cagr"] >= 8 and data["margin"] >= 25 and data["fcf"] > 0)
    weak_fund = (data["cagr"] <= 0 or data["margin"] < 20 or data["fcf"] <= 0)
    if strong_fund:
    ...
    speculative_label = "No strong meme/speculative signs" if not speculative_flags else " / ".join(speculative_flags)
