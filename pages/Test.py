...
# coding: utf-8
"""
Enhanced Entry Position Analyzer
...
NOTE: This file depends on your existing `imports` module which must provide
`ta` (technical functions) and any helper imports you had. It also uses yfinance
and scikit-learn. Adapt paths as needed.
"""
...
MIN_TRAIN_ROWS = {'4H': 150, '1D': 120, '1W': 40}
_DAYS = 21
...
FEATURES = [
    ...
    'CCI','KCu','KCl','Kasym','Kcount','STu','STl'
]
...
def get_current_price(ticker):
    try:
        t = yf.Ticker(ticker)
        data = t.history(period='1d', progress=False, auto_adjust=True)
...
def add_pivots(df, win=windows):
    for w in win:
        ...
        PP = (roll_high.max() + roll_low.min() + roll_close.apply(lambda x: x.values[-1])).div(3)
        df[f'PP_{w}'] = PP
        df[f'R1_{w}'] = 2*PP - roll_low.min()
        df[f'S1_{w}'] = 2*PP - roll_high.max()
        df[f'R2_{w}'] = PP + (roll_high.max() - roll_low.min())
        df[f'S2_{w}'] = PP - (roll_high.max() - roll_low.min())
    return df
...
def label_hit_prob_past(df, window=14, profit_target=0.05, stop_loss=0.05):
    ...
    for i in range(N):
        ...
        for j,p in enumerate(future):
            if tp_hit is None and p>=tp:
                tp_hit = j
            if sl_hit is None and p<=sl:
                sl_hit = j
...
def train_models(df, timeframe):
    required = [c for c in FEATURES if c in df.columns] + ['Hit_Label','Expected_Return','Expected_Loss']
    df_model = df.dropna(subset=required)
    if len(df_model) < MIN_TRAIN_ROWS.get(timeframe, 50):
...
def make_prediction(clf, model_ret, model_loss, scaler_cls, scaler_ret, scaler_loss, latest_row):
    # latest_row: single-row DataFrame
    # Ensure required features present
    feats = [c for c in FEATURES if c in latest_row.columns]
    if any(latest_row[feats].isnull().any()):
        return None
    Xlatest = scaler_cls.transform(latest_row[feats])
    probs = clf.predict_proba(Xlatest)[0]
    # build prob dict
...
def aggregate_timeframes(results):
    # results: dict of timeframe->prediction dict
    votes = {'Strong Buy':0,'Buy (Cautious)':0,'Avoid / Short bias':0,'Hold / Wait':0}
    for tf,p in results.items():
        lab = p.get('decision_label')
        votes[lab] = votes.get(lab,0)+1
    # Simple rule: if >=2 strong buy/buy then BUY
...
def plot_analysis2(ticker, df, entry_price, timeframe, prediction, assessment):
    ...
    if 'SMA1' in df.columns:
        ax1.plot(df.index, df['SMA1'], label='SMA1')
    if 'SMA2' in df.columns:
        ax1.plot(df.index, df['SMA2'], label='SMA2')
    last = df.iloc[-1]
    ax1.scatter(df.index[-1], entry_price, marker='o', label='Entry')
    if prediction:
        ax1.axhline(y=prediction['predicted_tp'], color='blue', linestyle='--', alpha=0.6)
        ax1.axhline(y=prediction['predicted_sl'], color='red', linestyle='--', alpha=0.6)
    ax1.legend()
    if 'RSI' in df.columns:
        ax2.plot(df.index, df['RSI'], label='RSI')
        ax2.axhline(70, linestyle='--')
        ax2.axhline(30, linestyle='--')
    plt.suptitle(f"{ticker} {timeframe} - {assessment}")
    plt.tight_layout()
    return fig
...
def main():
    st.title("📊 Entry Position Analyzer — Enhanced")
    col1,col2,col3 = st.columns(3)
    with col1:
        ticker = st.text_input('Ticker', value='TSLA').upper()
    with col2:
        entry_price = st.number_input('Entry Price ($)', min_value=0.0, value=0.0, step=0.1)
    with col3:
        user_gain = st.number_input('Expected Gain (%)', min_value=0.1, max_value=50.0, value=5.0)
        user_loss = st.number_input('Expected Loss (%)', min_value=0.1, max_value=50.0, value=4.5)
