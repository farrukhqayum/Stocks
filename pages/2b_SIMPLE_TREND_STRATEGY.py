...
st.title("🚀 Hedge Fund Momentum Strategy") 

# YOUR EXACT EXPANDER
with st.expander("📖 Real-Time Trade Logic", expanded=False):
    st.markdown("""
    ## 🎯 **HEDGE FUND ENTRY: Institutional Quality**
    
    **1. TREND STRENGTH (ADX > 25)** - Real institutional momentum
    **2. MOMENTUM (RSI > 50)** - Big money flowing in  
    **3. STRUCTURE (Close > EMA20)** - Above key level
    **4. VOLUME CONFIRMATION** - Institutions buying
    
    ## 🚪 **HEDGE FUND EXITS: Asymmetric Risk**
    
    **STOP: 1.5x ATR** - Volatility-adjusted protection
    **TARGET: NO LIMIT** - Let winners run 10x+
    **EXIT: EMA20 break ONLY** - Ride the full trend
    
    **Key: 1 big winner = 10 losers**
    """)
...
atr_mult_sl = col3.number_input("ATR SL Mult", value=1.5, min_value=1.0, max_value=3.0)
...
# YOUR EXACT FUNCTIONS (KEEP 3LB - IT WORKS)
...
# HEDGE FUND BACKTEST LOGIC
def run_hedgefund_backtest(df, initial_capital, sma_fast_len, rsi_len, atr_mult_sl):
    df_bt = df.copy()
    ...
    for i in range(len(df_bt)):
        ...
        # HEDGE FUND ENTRY: SIMPLE + POWERFUL
        entry_condition = (
            (row['ADX'] >= 25) &                    # Institutional trend strength
            (row['RSI'] >= 50) &                    # Big money momentum
            (curr_close >= row['EMA_FAST'])         # Above key structure
        )
        ...
        if position_shares == 0 and entry_condition:
            ...
            stop_price = entry_price - (atr_mult_sl * atr_value)  # HEDGE FUND: ATR stop
            ...
            # HEDGE FUND EXITS:
            if curr_low <= stop_price:  # ATR Stop Loss
                exit_price = stop_price
                exit_reason = 'ATR_Stop'
                exit_triggered = True
            elif curr_close < row['EMA_FAST']:  # Structure break
...
equity_series, final_capital, trades, df_bt = run_hedgefund_backtest(
...
# YOUR EXACT PLOTTING - ALL DETAILS RESTORED
...
ax1.scatter(entry_signals.index, entry_signals['Close'], color='magenta', marker='d', s=10, 
           label=f'Hedge Signals ({len(entry_signals)})', alpha=0.8, zorder=5)

# YOUR EXACT TRADE MARKERS
if len(trades) > 0:
    trades_plot = pd.DataFrame(trades)
    ax1.scatter(trades_plot['entry_date'], trades_plot['entry_price'], 
               color='blue', marker='o', s=15, label=f'Entries ({len(trades)})', alpha=0.8, zorder=2, edgecolors='darkblue', linewidths=0.7)
    ...
    if len(exits) > 0:
        ...
        if len(winners) > 0:
            ax1.scatter(winners['exit_date'], winners['exit_price'], color='green', marker='o', s=20, label=f'Winners ({len(winners)})', alpha=0.8, zorder=1, edgecolors='darkgreen', linewidths=0.7)
        if len(losers) > 0:
            ax1.scatter(losers['exit_date'], losers['exit_price'], color='red', marker='o', s=20, label=f'Losses ({len(losers)})', alpha=0.8, zorder=1, edgecolors='black', linewidths=0.7)

ax1.set_title(f'{ticker} - Hedge Fund Momentum | Return: {total_return:+.1f}% | {len(trades)} Trades', 
...
col1.metric("ADX Strength", "✅" if latest['ADX'] >= 25 else "❌", f"{latest['ADX']:.1f}")
col2.metric("RSI Momentum", "✅" if latest['RSI'] >= 50 else "❌", f"{latest['RSI']:.1f}")
col3.metric("Structure", "✅" if latest['Close'] >= latest['EMA_FAST'] else "❌", f"EMA20: {latest['EMA_FAST']:.1f}")
col4.metric("HEDGE SIGNAL", "🟢 ENTER" if live_entry else "🔴 WAIT")

if live_entry:
    st.success("🎯 INSTITUTIONAL ENTRY SIGNAL!")
    st.balloons()
...
st.caption(f"Hedge Fund Strategy | {datetime.now().strftime('%Y-%m-%d %H:%M')}")
