...
if df_daily is None:
    ...
    st.stop()
try:
    # Method 1: Use period for hourly (MOST RELIABLE)
    ...
    st.warning(f"Period download failed: {e}, trying alternative method...")
    
    # Method 2: Fallback to start/end with extended range
    start_hourly = datetime.today() - timedelta(days=10)
    end_hourly = (datetime.today() + timedelta(days=1)).strftime("%Y-%m-%d")
...
if df_hourly is not None and not df_hourly.empty:
    ...
    hours_old = (today - last_date).total_seconds() / 3600
    if hours_old > 2:
