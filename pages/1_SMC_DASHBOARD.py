...
def draw_smc_box(ax, df, zones):
    ...
    zone_lines.append((f"  BULL FVG: {'✓' if has_bull_fvg else '✗'}", yesno_color(has_bull_fvg)))
    zone_lines.append((f"  BEAR FVG: {'✓' if has_bear_fvg else '✗'}", yesno_color(has_bear_fvg)))
    zone_lines.append((f"  Inside Bull: {'✓' if inside_bull else '✗'}", yesno_color(inside_bull)))
    zone_lines.append((f"  Inside Bear: {'✓' if inside_bear else '✗'}", yesno_color(inside_bear)))
    zone_lines.append((f"  1stTouch Bull: {'✓' if first_touch_bull else '✗'}", yesno_color(first_touch_bull)))
    zone_lines.append((f"  1stTouch Bear: {'✓' if first_touch_bear else '✗'}", yesno_color(first_touch_bear)))
