...
def del_old_files(directory, days, exclude_extensions=None, dry_run=False):
    ...
    for filename in os.listdir(directory):
        ...
        if file_mtime < cutoff_time:
            if dry_run:
                st.write(f"[Dry Run] Would delete: {filepath}")
            else:
                try:
                    os.remove(filepath)
...
def plot_single_ticker(ticker, df, df_results, _window=14):
    ...
    if predictions.empty:
        st.write(f"No prediction results found for ticker {ticker}, skipping plot.")
        return
...
def MakePredictions():
    ...
    for ticker in TICKERS:
        try:
            ...
            if len(df_model) < _Nr:
                st.write(f"Skipping {ticker} due to insufficient data after dropna.")
                continue
            ...
            if latest[FEATURES].isnull().values.any():
                st.write(f"Skipping {ticker} for NULL Features")
                null_features = latest[FEATURES].iloc[0].isnull()
                st.write(f"NaN features for {ticker}: {list(null_features[null_features].index)}")
                continue
...
def plot_price_with_overlaps(df):
    ...
    fig, (ax, bx) = plt.subplots(2, 1, figsize=(12, 6), dpi = 300, sharex=True, gridspec_kw={'height_ratios': [3, 1]})
    
    ax.plot(df.index, df['Close'], label='Close Price', alpha=0.5, color='gray', linewidth=1.2, zorder=1)
    ax.plot(df.index, df['SMA1'], label=f'SMA{int(_DAYS*0.5)}', color='gold', alpha=0.5, linewidth=1.2)
    ax.plot(df.index, df['SMA2'], label=f'SMA{int(_DAYS*2)}', color='red', alpha=0.5, linewidth=1.2, linestyle='--')

    # SL (1)
    bear_points = df[df['Hit_Label'] == 1]
    ax.scatter(bear_points.index, bear_points['Close'], color='red', marker='v', s=_s , alpha=0.5, label='SL', zorder=2)
    
    # TP (2)
    bull_points = df[df['Hit_Label'] == 2]
    ax.scatter(bull_points.index, bull_points['Close'], color='green', marker='^', s=_s , alpha=0.5, label='TP', zorder=3)

    # Short (3)
    short_points = df[df['Hit_Label'] == 3]
    ax.scatter(short_points.index, short_points['Close'], color='orange', marker='o', s=_s , alpha=0.5, label='Hold', zorder=4)

    # Neutral (4)
    neutral_points = df[df['Hit_Label'] == 4]
    ax.scatter(neutral_points.index, neutral_points['Close'], color='red', marker='.', s = _s, alpha=0.5, label='ShortL', zorder=5)

    df['90D_High'] = df['Close'].rolling(window=90).max()
    ax.plot(df.index, df['90D_High'], label='90-Day High', color='blue', linewidth=1.5, linestyle=':')
    
    # Neutral (0)
    neutral_points = df[df['Hit_Label'] == 0]
    #ax.scatter(neutral_points.index, neutral_points['Close'], color='black', marker='.', s = _s*2, alpha=0.2, label='Neutral', zorder=6)
    rsi_ = df['RSI'].rolling(3).mean()
    rsi_sma = df['RSI'].rolling(20).mean()
    bx.grid(color='lightgray', linestyle='-', linewidth=0.5, alpha=0.5)
    bx.plot(df.index, rsi_, label='RSI', color='gray', linewidth=1.5, alpha=0.5)
    bx.plot(df.index, rsi_sma, label='RSI SMA', color='gold', linewidth=1.2, alpha=0.7)

    # Fill RSI above 52 (green) and below 40 (red)
    bx.fill_between(df.index, rsi_, 52,
                    where=(df['RSI'] > 52),
                    facecolor='green', alpha=0.15)
    bx.fill_between(df.index, rsi_, 40,
                    where=(df['RSI'] < 40),
                    facecolor='red', alpha=0.15)

    # Horizontal RSI Levels
    bx.axhline(80, color='green', linewidth=1, linestyle='dotted', alpha=0.3)
    bx.axhline(20, color='red', linewidth=1, linestyle='dotted', alpha=0.3)
    bx.axhline(40, color='brown', linewidth=1, linestyle='dashed', alpha=0.3)
    bx.axhline(52, color='gray', linewidth=1.2, linestyle='dashed', alpha=0.3)
    bx.set_ylim(0, 100)
    bx.yaxis.tick_right()
    bx.yaxis.set_label_position("right")
    bx.set_ylabel('RSI')
    
    ax.set_title('Price Chart with Hit_Label QC')
    ax.set_xlabel('Date')
    ax.set_ylabel('Price')
    ax.legend()
    ax.grid(True)
...
# Example usage
plot_price_with_overlaps(dfs.get('CRM'))
